...
export default class IssueClient {
  ...
  public async getIssues(page?: number, perPage?: number): Promise<Issue[]> {
    const res: ResIssue[] = await fetch(
      `${this.httpClient}?page=${page}&per_page=${perPage}`,
      {
        headers: {
  ...
  public async getIssue(number: number): Promise<Issue> {
    const issues = await this.getIssues(0);
